...
sub post_ops_mod_euca_conf{
	...
	if( $DISTRO eq "UBUNTU" ){
		...
		if( does_It_Have($ROLL, "NC") ){
			if( $ENV{'BUILD_NETWORK'} eq "MANAGED" || $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" || $ENV{'BUILD_NETWORK'} eq "STATIC" ){
	...
	if( does_It_Have($ROLL, "CC") ){
		...
		if( $ENV{'BUILD_NETWORK'} eq "MANAGED" || $ENV{'BUILD_NETWORK'} eq "MANAGED-NOVLAN" ){
			...
			# STATIC MODE setup
			my $subnet_ip = get_subnet_ip();
			my_sed( "#VNET_SUBNET=\"192.168.1.0\"", "VNET_SUBNET=\"$subnet_ip\"", $euca_conf );

			my_sed( "#VNET_NETMASK=\"255.255.255.0\"", "VNET_NETMASK=\"255.255.0.0\"", $euca_conf );
			my_sed( "#VNET_BROADCAST=\".*\"", "VNET_BROADCAST=\"192.168.7.255\"", $euca_conf );
